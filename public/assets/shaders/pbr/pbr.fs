...
void main() {
    vec3 albedo = pow(texture(u_albedo_map, v_uv).rgb, vec3(2.2));
    ...
    vec3 N = normalize(mix(normalize(v_normal), texture(u_normal_map, v_uv).rgb * 2.0 - 1.0, 0.5));
    ...
    vec3 R = reflect(-V, N);
    float maxMip = 5.0;
    vec3 prefilteredColor = textureLod(u_prefilter_map, R, roughness * maxMip).rgb;
